...
const tabs = ['Personal', 'Hero', 'Navbar', 'About', 'Footer', 'Theme', 'Security'];
...
export default function SettingsManager() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        {tabs.map(t => (
          <button key={t} onClick={() => setActiveTab(t)}
            className={`px-4 py-2 rounded-xl text-xs font-heading font-black tracking-widest uppercase transition cursor-pointer ${activeTab === t ? 'bg-mustard text-charcoal shadow-lg' : 'bg-[#2D2F33] text-slate-400 hover:text-white'}`}>
            {t}
          </button>
        ))}
      </div>
      ...
      {activeTab === 'Security' ? (
        <form onSubmit={handlePasswordSubmit} className="glass-card p-6 space-y-5">
          ...
          <div className="pt-2">
            <SaveBtn loading={changingPassword} label="Update Password" />
          </div>
        </form>
      ) : (
        <form onSubmit={handleSave} className="glass-card p-6 space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input 
                  ...
                  className="block w-full text-xs text-slate-400 file:mr-3 file:py-1.5 file:px-3 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-mustard file:text-charcoal hover:bg-mustard-hover cursor-pointer"
                />
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-2">
                ...
                <button
                  ...
                  className="w-full py-2 rounded-xl border border-dashed border-mustard text-xs font-semibold text-mustard hover:bg-mustard/10 transition cursor-pointer"
                >
          ...
          {activeTab === 'Navbar' && (<>
            ...
            <div className="border-t border-slate-800/80 pt-6 mt-6">
              <h3 className="text-sm font-bold text-white mb-4 font-heading">Navbar Navigation Links</h3>
              <div className="space-y-3">
                {(form.navbar?.links || []).map((link, idx) => (
                  <div key={idx} className="flex gap-3 items-center bg-charcoal-medium/40 p-3 rounded-xl border border-charcoal-light/50">
                    ...
                    <button 
                      ...
                      className="p-2 rounded-xl bg-red-500/10 text-red-400 hover:bg-red-500/20 transition cursor-pointer self-center mt-6 text-xs font-bold uppercase"
                    >
                      Delete
                    </button>
                  </div>
                ...
                <button 
                  ...
                  className="w-full py-2.5 rounded-xl border border-dashed border-charcoal-light text-xs font-semibold text-slate-400 hover:text-slate-200 hover:border-slate-700 transition cursor-pointer bg-charcoal-medium/20 font-heading uppercase"
                >
                  + Add Navigation Link
                </button>
              </div>
          ...
          {activeTab === 'About' && (<>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <Input label="Tools Mastered" value={form.about?.toolsMastered || ''} onChange={e => set('about.toolsMastered', e.target.value)} />
            </div>
            <Textarea label="About Bio Text" value={form.about?.bio || ''} onChange={e => set('about.bio', e.target.value)} rows={4} />
          </>)}

          {activeTab === 'Footer' && (<>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Input label="Footer Tagline" value={form.footer?.tagline || ''} onChange={e => set('footer.tagline', e.target.value)} />
              <Input label="Copyright Override" value={form.footer?.copyright || ''} onChange={e => set('footer.copyright', e.target.value)} />
            </div>
            <div className="border-t border-slate-800/80 pt-6 mt-6">
              <h3 className="text-sm font-bold text-white mb-4 font-heading">Footer Quick Links</h3>
              <div className="space-y-3">
